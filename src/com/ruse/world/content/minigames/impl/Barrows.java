...
public class Barrows {
	 ...
	 * @param spawnPos The {@link Position} of the NPC being spawned.
	 */
				...
				if (player.getInventory().getSlot(items.getId()) > 0) {
					if (items.getId() == brokenBarrow[1]) {
						if (totalCost + 45000 > money) {
							...
							totalCost += 45000;
							player.getInventory().setItem(player.getInventory().getSlot(items.getId()), new Item(brokenBarrow[0], 1));
	...
	public static boolean isBarrowsNPC(int id) {
		for (int[] coffinAndBrother : COFFIN_AND_BROTHERS) {
			if (coffinAndBrother[1] == id)
				return true;
		}
		return false;
	}
