...
public class PuroPuro {
	...
	public static void spawn() {
		...
		for (int[] impling : implings) {
			NPC n = new NPC(impling[0], new Position(impling[1], impling[2]));
	...
	public static void catchImpling(Player player, final NPC imp) {
		...
		if(player.getInterfaceId() > 0 || implingData == null || !imp.isRegistered() || !player.getClickDelay().elapsed(2000))
			return;
		...
		boolean sucess = player.getSkillManager().getCurrentLevel(Skill.HUNTER) <= 8 || Misc.getRandom(player.getSkillManager().getCurrentLevel(Skill.HUNTER) / 2) > 1;
		if(sucess) {
			if(imp.isRegistered()) {
				...
				player.getSkillManager().addExperience(Skill.HUNTER, implingData.XPReward * GameSettings.BaseImplingExpMultiplier);
	 ...
	 */
	public static void goThroughWheat(final Player player) {
	 ...
	 * @param jar		The jar the player is looting
	 * @param jarData	Settings for the jar
	 */
