...
public class Misc {
	...
	public static boolean needsNewSalt(String currentSalt) {
		...
		if (bcr >= 4) { //bcrypt requires minimum 4 rounds, max 30
			return bcr != GameSettings.BCRYPT_ROUNDS;
		}
		return true;
	}
	...
	public static boolean validString(String target) {
		...
		return !target.equalsIgnoreCase("null");
	}
...
//		str1.replace("_", " ");
	...
	public static int getMinutesElapsed(int minute, int hour, int day, int year) {
		...
		if (i.get(Calendar.YEAR) == year) {
			if (i.get(Calendar.DAY_OF_YEAR) == day) {
				...
				return (i.get(Calendar.HOUR_OF_DAY) - hour) * 60 + (59 - i.get(Calendar.MINUTE));
			}

			int ela = (i.get(6) - day) * 24 * 60 * 60;
			return Math.min(ela, 2147483647);
		}
	...
	public static boolean easter(int year) {
		return getMonth() == 3 && getYear() == year && getDayOfMonth() >= 12 && getDayOfMonth() <= 29;
	}
	...
	public static boolean isWeekend() {
		Calendar c = Calendar.getInstance(TimeZone.getTimeZone("America/New_York"));
		return c.get(Calendar.DAY_OF_WEEK) == Calendar.FRIDAY || c.get(Calendar.DAY_OF_WEEK) == Calendar.SATURDAY || c.get(Calendar.DAY_OF_WEEK) == Calendar.SUNDAY;
	}
	 ...
	 * @see {@link Misc#exclusiveRandom(int)}.
	 */
	 ...
	 * @param buffer 	The ChannelBuffer to read string from.
